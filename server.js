...
app.post('/login', (req, res) => {
    ...
    const sql = `SELECT id, password_hash FROM patients WHERE email = ?`;
    db.query(sql, [loginemail], (err, rows) => {
        ...
        // If the login is successful, store patient ID in the session
...
app.get('/doctor', (req, res) => {
    ...
    fs.readFile(filePath, (err, data) => {
        if (err) {
            res.status(500).send('Error loading doctor.html');
        } else {
            res.writeHead(200, { 'Content-Type': 'text/html' });
            res.end(data); // Serve the doctor page
        }
    });
});
